...
export function ClientLayout() {
    ...
    // Các trang có ảnh hero tối → header trong suốt + chữ trắng khi chưa scroll
    const darkHeroRoutes = ['/', '/rooms', '/news', '/about', '/attractions', '/memberships', '/services'];
    const hasDarkHero = darkHeroRoutes.some(p =>
        ...
        <div className="min-h-screen flex flex-col font-['Manrope'] bg-[#F9FAFB] text-slate-800">
            ...
            <header
                className={`fixed top-0 w-full z-50 transition-all duration-500 ease-in-out ${
                    isScrolled
                        ? 'bg-white/95 backdrop-blur-xl shadow-md py-3 border-b border-slate-200'
                        : isTransparent
                            ? 'bg-black/40 backdrop-blur-sm py-6'
                            : 'bg-white/95 backdrop-blur-xl shadow-sm py-4 border-b border-slate-200'
                }`}
            >
                <div className="max-w-[1400px] mx-auto px-6 sm:px-8 lg:px-12">
                    <div className="flex justify-between items-center">
                        ...
                        <Link
                            to="/"
                            className={`text-2xl font-extrabold tracking-tight transition-colors hover:text-cyan-500 ${
                                isTransparent ? 'text-white drop-shadow-md' : 'text-black'
                            }`}
                        >
                        ...
                        {/* Desktop Nav */}
                        <nav className={`hidden lg:flex gap-6 items-center px-8 py-3 rounded-full border transition-all duration-500 ${
                            isTransparent
                        ...
                        <div className="hidden lg:flex items-center gap-4">
                            {isClientLoggedIn ? (
                                <Link
                                    ...
                                    className={`text-sm font-semibold transition-colors ${
                                        isTransparent ? 'text-white hover:text-cyan-300' : 'text-black hover:text-cyan-600'
                                    }`}
                                >
                                    My Account
                                </Link>
                            ...
                            <Link
                                to="/rooms"
                                className="bg-cyan-600 hover:bg-cyan-500 text-white px-6 py-2.5 text-xs tracking-widest uppercase font-bold transition-all"
                            >
                                Book Now
                            </Link>
                        </div>
                        ...
                        <div className="lg:hidden flex items-center">
                            <button
                                ...
                                className={`p-2 border transition-colors ${
                                    isTransparent
                                        ? 'text-white border-white/40 bg-white/10 backdrop-blur hover:bg-white/20'
                                        : 'text-black border-black hover:bg-slate-100'
                                }`}
                ...
                {/* Mobile Menu Drawer */}
                {mobileMenuOpen && (
                    <div className="lg:hidden absolute top-full left-0 w-full bg-slate-800 border-t border-slate-700 shadow-2xl py-4 px-6 flex flex-col gap-1 z-40">
                        {navLinks.map((link) => (
                            <Link
                                key={link.name}
                                to={link.href}
                                className={`text-base font-semibold px-4 py-3 transition-colors ${
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-700 flex flex-col gap-3">
                            <Link to="/rooms" className="block w-full text-center bg-cyan-600 hover:bg-cyan-500 text-white py-3 text-sm tracking-widest uppercase font-bold transition-colors">
                                Book Your Stay
                            </Link>
                            <Link to={isClientLoggedIn ? "/account" : "/login"} className="block w-full text-center border border-slate-600 text-slate-300 hover:text-white py-3 mt-1 text-sm tracking-widest uppercase font-bold transition-colors">
                                {isClientLoggedIn ? "Account Dashboard" : "Sign In / Register"}
                            </Link>
                            <Link to="/admin" className="block w-full text-center text-slate-500 hover:text-cyan-400 py-4 transition-colors text-sm">
                                Staff Portal
                            </Link>
                        </div>
                    </div>
                        ...
                        <div className="lg:col-span-1">
                            ...
                            <form className="flex flex-col gap-3">
                                ...
                                <button
                                    ...
                                    className="bg-white hover:bg-cyan-50 text-slate-900 px-4 py-3 rounded-none text-sm uppercase tracking-widest font-bold transition-colors w-full"
                                >
                                    Subscribe
                                </button>
                            </form>
